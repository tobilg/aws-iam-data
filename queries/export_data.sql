...
COPY (SELECT * FROM aws_actions ORDER BY action_id ASC, service_id ASC) TO 'data/csv/aws_actions.csv' WITH (HEADER 1, DELIMITER ',');
COPY (SELECT * FROM aws_actions ORDER BY action_id ASC, service_id ASC) TO 'data/parquet/aws_actions.parquet' (FORMAT 'parquet', COMPRESSION 'SNAPPY');
...
COPY (SELECT * FROM aws_resource_types ORDER BY resource_type_id ASC, service_id ASC) TO 'data/csv/aws_resource_types.csv' WITH (HEADER 1, DELIMITER ',');
COPY (SELECT * FROM aws_resource_types ORDER BY resource_type_id ASC, service_id ASC) TO 'data/parquet/aws_resource_types.parquet' (FORMAT 'parquet', COMPRESSION 'SNAPPY');
...
COPY (SELECT * FROM aws_actions_resource_types ORDER BY action_resource_type_id ASC) TO 'data/csv/aws_actions_resource_types.csv' WITH (HEADER 1, DELIMITER ',');
COPY (SELECT * FROM aws_actions_resource_types ORDER BY action_resource_type_id ASC) TO 'data/parquet/aws_actions_resource_types.parquet' (FORMAT 'parquet', COMPRESSION 'SNAPPY');
