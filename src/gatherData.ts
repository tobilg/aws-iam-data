  ...
  for (let rowNum = 0; rowNum < actionTableRows.length; rowNum++) {
    ...
    if (!action || (rowNum === nextActionRow)) {
      ...
      if (actionNameNode) {
        ...
        action.apiReferenceUrl = actionNameNode.attributes['href'];
      ...
      resourceType.conditionKeys = conditionKeyNodes.map(conditionKeyNode => conditionKeyNode.textContent.trim());

      const dependentActionNodes = rowCellNodes[rowCellNodes.length-1].querySelectorAll('p');
      resourceType.dependentActions = dependentActionNodes.map(dependentActionNode => dependentActionNode.textContent.trim());
...
const getTopicPage = async (topic: Topic): Promise<ServiceAuthReference> => {
  // Download topic HTML
  const html = await downloadAsHTML(topic.authReferenceUrl);
